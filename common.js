...
(function () {
  ...
  /* V0.4 3항 - 한 번에 하나만. 본인 확인·비밀번호 창이 열려 있으면 뒤로 미룹니다.
     ★경고를 버리지 않습니다 - 5초 뒤 다시 겁니다(판정 MST2-20260818_19 §4). */
  function showIdleWarn() {
    var m = document.getElementById('idleModal'); if (!m) return;
    var busy = ['reauthModal', 'pwModal'].some(function (k) {
      var x = document.getElementById(k); return !!(x && x.classList.contains('open'));
    });
    if (busy) { clearTimeout(warnT); warnT = setTimeout(showIdleWarn, 5000); return; }
    m.classList.add('open');
  }
  /* 유휴 경고를 사람이 받아들였을 때 - ★타이머가 꺼져 있어도 닫힙니다 */
  function idleAck() { hideIdleWarn(); if (_idleOn) resetIdle(); }
  ...
  /* V0.4 2항 - 제외 화면에서는 ★뜨지 않게가 아니라 ★만들지 않습니다.
     규칙이 빠져도 안 보이게 하려는 것입니다 - 이번 사고가 그 규칙 누락이었습니다.
     판정 MST2-20260818_19 §4 · 상신 MK-20260818_16 §3-2 */
  var M017_OFF = ['gate'];
  function m017Enabled() {
    var el = document.getElementById('edmsHeader');
    var k = el ? (el.getAttribute('data-system') || 'gate') : 'gate';
    return M017_OFF.indexOf(k) < 0;
  }
  ...
  /* V0.4 4항 탈출 경로 - 세 창을 ★단추·ESC·바깥 눌러 닫습니다.
     ★핸들러를 새로 만들지 않고 ★그 창의 나가는 단추를 대신 누릅니다 -
       그러면 기다리던 약속이 ★원래 자리에서 풀립니다(두 벌 관리 0 · 열리는 쪽 실패 0).
     ★유휴 단추는 ★타이머가 꺼져 있어도 듣습니다 - 사람이 갇힌 원인이 그것이었습니다.
     판정 MST2-20260818_19 §4 ㉠ */
  var EXIT_BTN = { idleModal: 'idleStay', reauthModal: 'reauthCancel', pwModal: 'pwClose' };
  var EXIT_ORDER = ['idleModal', 'reauthModal', 'pwModal'];   // 위에 덮인 것부터
  function exitModal(id) {
    var b = document.getElementById(EXIT_BTN[id]);
    if (b) { b.click(); return true; }
    var m = document.getElementById(id); if (m) m.classList.remove('open');
    return false;
  }
  function bindModalExit() {
    document.addEventListener('keydown', function (ev) {
      if (ev.key !== 'Escape') return;
      for (var i = 0; i < EXIT_ORDER.length; i++) {
        var m = document.getElementById(EXIT_ORDER[i]);
        if (m && m.classList.contains('open')) { exitModal(EXIT_ORDER[i]); return; }
      }
    });
    EXIT_ORDER.forEach(function (id) {
      var m = document.getElementById(id); if (!m) return;
      m.addEventListener('click', function (ev) { if (ev.target === m) exitModal(id); });   // 바깥만
    });
    var a = document.getElementById('idleStay'), b = document.getElementById('idleClose');
    if (a) a.addEventListener('click', idleAck);
    if (b) b.addEventListener('click', idleAck);
  }
